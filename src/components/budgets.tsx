...
export default function Budgets() {
    ...
    const data = useMemo(() => {
        const spent = budgets.map(b => b.transactions.reduce((accumulator, currentValue) => {
          const amt = accumulator + currentValue.amount;
          return amt
        }, 0) * -1);
        
        const b = budgets.map((budget, index) => {
    ...
    return(
        <div className="col-start-7 col-span-5 bg-light p-4 rounded-lg row-start-1 row-span-2">
            ...
            <div className='mt-7 flex justify-between'>
                <div className="relative">
                    ...
                    <Pie data={data} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={70} outerRadius={110}>
                        {data.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={data[index].theme} />
                        ))}
                    </Pie>
                    ...
                    <p className='text-light-text'>of ${data.reduce((accumulator, currentValue) => {
                ...
                <div>
                    { data.slice(0, 4).map((budget, index) => (
